...
extern openni::VideoFrameRef  mColorFrame;
extern openni::VideoFrameRef mDepthFrame;

double sbGlobleBubbleNumber=0;
double sbGlobleBubbleSize=0;
double sbGlobleGrayMean=0;
double sbGlobleLevel =0;
...
QString ImageValue::getsbLevel(void) const
{
    return m_sbLevel;
}
void ImageValue::setsbLevel(const QString sbLevel)
{
    m_sbLevel=sbLevel;
    emit sbLevelChanged();
}
...
void ImageValue::getSbImageValue()
{
    Histogram h;//创建图像增强算法实例;
    WatershedAlgorithm myWatershed;	// 创建分水岭算法实例;

    //2.read image from ColorFrame
    int nr=mColorFrame.getHeight();
    int nc=mColorFrame.getWidth();
    int sbimagesize=nc*nr;
    cv::Mat mImageRGB(nr, nc, CV_8UC3, (void*)mColorFrame.getData() );
    //convert form RGB to BGR
    cv::Mat sbopencvimage;
    cv::cvtColor( mImageRGB, sbopencvimage, CV_RGB2BGR );
...
//    UCC_GetBitmapImage(sbcamerazz.getSbCameraID(),UINT8P_CAST(sbopencvimage.data),1000);
  ...
  //  imshow("adjust image",opencvimage_imadjust);
   ...
   // imshow("wtershed result1",WSS);
    imshow("wtershed result2",sbWSS+sbopencvimage_gray);//显示分割效果
    ...
    uint16_t* pDepthRow = (uint16_t*)mDepthFrame.getData();
//    cv::Mat mImageDepth(
//              mDepthFrame.getHeight(), mDepthFrame.getWidth(),
//              CV_16UC1, (uint16_t*)mDepthFrame.getData() );

    sbimagesize=0;
    long testdepth=0;
    for (int j=0;j<nr;j++)
        {
//            uchar* data=mImageDepth.ptr<uchar>(j);
            uint16_t* data=pDepthRow;
            for (int i=0;i<nc;i++)
            {
//               testdepth[i]=data[i];
                if(data[i]!=0)
                {
                    testdepth=data[i];
                    sbsumlevel=sbsumlevel+data[i];
                    sbimagesize=sbimagesize+1;
                }
            }
        }
    sbGlobleLevel=sbsumlevel/sbimagesize;
    ...
    setsbLevel(QString::number(sbGlobleLevel));
}
